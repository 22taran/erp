...
log_file_name = "easy-install__{0}__{1}.log".format(
...
def setup_log_stream(args):
    ...
    if not args.verbose:
        ...
        print(
            "Install script run at {0} on {1}\n\n".format(
                execution_time, execution_day
            ),
            file=log_stream,
        )
...
def check_distribution_compatibility():
    ...
    if dist_name in supported_dists:
        if float(dist_version) in supported_dists[dist_name]:
            ...
            log(
                "Install on {0} {1} instead".format(
                    dist_name, supported_dists[dist_name][-1]
                ),
                level=3,
            )
    else:
        log(
            "Sorry, the installer doesn't support {0}. Aborting installation!".format(
                dist_name
            ),
            level=2,
        )
...
def get_distribution_info():
    ...
    if platform.system() == "Linux":
        if (
            int(platform.python_version_tuple()[0] + platform.python_version_tuple()[1])
            <= 3.7
        ):
...
def could_not_install(package):
    raise Exception(
        "Could not install {0}. Please install it manually.".format(package)
    )
    ...
    if not ignore_prompt:
        ...
        while pass_set:
            ...
            if not mysql_root_password:
                ...
                conf_mysql_passwd = getpass.unix_getpass(
                    prompt="Re-enter mysql root password: "
                )

                if (
                    mysql_root_password != conf_mysql_passwd
...
def parse_commandline_args():
    ...
    parser.add_argument(
        "--without-site", dest="without_site", action="store_true", default=False
    )
    parser.add_argument(
        "--verbose",
        dest="verbose",
    ...
    parser.add_argument(
        "--user", dest="user", help="Install frappe-bench for this user"
    )
    parser.add_argument(
        "--bench-branch",
        dest="bench_branch",
